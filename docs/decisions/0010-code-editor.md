...
- The editor’s accessible name carries the component’s name and the example’s index (“Code editor for Button example 2”): Playground passes `exampleName` and `exampleIndex` into the slot props, two optional keys added to the contract at integration time.
