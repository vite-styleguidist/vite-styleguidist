...
**A component is more than its own file** (amended, see the consequences). A parse reads across files — `Button.propTypes = buttonProps` imported from a sibling module, the interface a `.tsx` component's props extend — so an entry also stores the sha256 of every *other* file its parse read, and is discarded as soon as one of them differs. The list comes from the parse itself: react-docgen is given an importer whose caches Styleguidist owns, so the files it followed are exactly the files recorded. A `propsParser` of your own cannot be asked what it read, so the component's own relative imports, followed transitively, stand in for it (`getImportedFiles()`); a type reached through a path alias or a package is outside that set and is documented as the limit of the option.
...
- **Two style guides rooted in the same folder share one cache file.** The location follows Vite's `cacheDir`, which follows the Vite root, which is the config file's folder — so `styleguide.config.js` and `styleguide.config.docs.js` side by side use the same file. If they differ in a parse-relevant option they will keep discarding each other's entries: always correct, never stale, just never warm. Naming the file after the fingerprint would fix it and would leave an 8 MB file behind for every option a single project ever changes, which is the worse failure. Give the second guide its own `viteConfig.cacheDir` if it matters.
...
- **Amended after release: a component's own imports were not part of its key either**, which is a different and much worse bug than the one above — it was the default configuration, and both parsers read across files. Editing only the module a component's `propTypes` (or its props interface) live in left the component's own bytes unchanged, so every later build, and the dev server with it, served the previous documentation for ever. The key could not be widened to cover it, because which files a parse reads is only known once it has run: they are stored with the entry and revalidated on every read instead. Two things fell out of the same change — react-docgen's default importer keeps every file it has followed parsed in a process-wide cache that is never cleared, so a running dev server was serving stale documentation even with `cache: false`; and the files a parse read are now watched, so editing one of them hot-updates the components that read it. See `getImportedFiles()` for what the approximation for a custom parser can and cannot see. The price is a stat and, when it has changed, a hash of each of those files on every cache read, memoized per run because a design system's components share them: measured on the 350-component tree against the same tree without it, in one session, a warm build goes from 1060 ms to 1090 ms and a cold one is unchanged at 1910–1920 ms.
