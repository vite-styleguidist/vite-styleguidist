...
- A style guide built in a project whose `react-dom` is later upgraded across the 17/18 boundary picks the other branch on the next build, with no configuration: the choice, and where the `react-dom` behind it was found, are logged in verbose mode (`Found react-dom <version> in the project, mounting with ...`) and reported by `styleguidist doctor`.
