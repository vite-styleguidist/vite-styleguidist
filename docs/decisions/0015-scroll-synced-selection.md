...
5. **Following a link wins over geometry until the reader scrolls.** A `hashchange` to a watched id selects it immediately and pins it; the pin is released by a `wheel` or `touchmove`, or by any scroll arriving more than 250 ms after the navigation (the keyboard and the scrollbar send nothing else). Without it, clicking an entry in the last screenful would highlight the last one instead, because the page cannot scroll far enough to put the clicked anchor on the activation line.
